...
const API_ARUBA =
  "https://www.agentiplusdb.net/ottica-api/admin_promozioni.php";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, errore: "Errore caricamento promozioni." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, errore: "Errore aggiornamento promozioni." },
      { status: 500 }
    );
  }
}
