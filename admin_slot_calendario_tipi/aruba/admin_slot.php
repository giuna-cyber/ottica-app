...
if ($azione === "genera") {
    ...
    try {
        ...
        out([
            ...
            "creati" => $creati,
            "saltati" => $saltati,
            "messaggio" => "Generazione completata: $creati creati, $saltati già presenti."
        ]);
    } catch (Throwable $e) {
...
if ($azione === "disponibilita") {
    ...
    try {
        ...
        out(["ok" => true, "messaggio" => "Disponibilità aggiornata."]);
    } catch (Throwable $e) {
...
if ($azione === "elimina") {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            DELETE FROM slot_appuntamenti
            WHERE id = ?
        ");
        $stmt->execute([$id]);

        out(["ok" => true, "messaggio" => "Slot eliminato."]);
    } catch (Throwable $e) {
        out(["ok" => false, "errore" => "Errore durante l'eliminazione."], 500);
    }
}

if ($azione === "elimina_multipli") {
    $idsRaw = $input["ids"] ?? [];

    if (!is_array($idsRaw) || count($idsRaw) === 0) {
        out(["ok" => false, "errore" => "Seleziona almeno uno slot."], 400);
    }
    ...
    if (count($ids) === 0) {
        out(["ok" => false, "errore" => "Selezione non valida."], 400);
    }
    ...
    try {
        ...
        out([
            ...
            "messaggio" =>
                $protetti > 0
                    ? "$eliminati slot eliminati. $protetti non eliminati perché collegati ad appuntamenti attivi."
                    : "$eliminati slot eliminati."
        ]);
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
        ...
        out(["ok" => false, "errore" => "Errore durante l'eliminazione multipla."], 500);
    }
}
